...
var timer = setInterval( clueTwo, 1000); //timer has function running every second
var timer = setInterval( keyCheck, 1000); //timer has function running every second
...
qRandom(); //calls random function when page is loaded so that question mark locations randomize

//letter functions make letters appear when user finds and clicks question marks
...
//calls calculator function to generate a number between 1 and 4; alters placement of question marks within their div grid using pre-made CSS classes.
function qRandom(){
	Calculator();
	console.log(x);
...
//When all letters are discovered, question marks are made invisible, background is changed, and second hint is revealed
function clueTwo(){
...
//If letters are correctly arranged, key image replaces hint 2 
function keyCheck(){
...
//Allows user to cycle through images
function nextImg(el){
...
//Opens second puzzle page when called
function nextPage(){
